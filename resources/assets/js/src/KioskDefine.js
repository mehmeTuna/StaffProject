...
export default class KioskDefine extends React.Component{
    ...
    async KioskAddName(){
        if(this.state.kioskName === ''){
          ...
          const {data} = await axios.post('/business/kiosk/add', {name: this.state.kioskName});
          console.log(data);
          ...
          if(data.status === true){
            sweet.fire('Kiosk eklendi artik giris cikis  islemlerinde kullanilabilir').then(()=> this.setState({ redirect: `/${this.props.data.username}/kiosk/list`}));
          }
    }
    ...
    render(){
        ...
        return(
            <React.Fragment>
                ...
                {this.state.tutorial === 0 &&  <div className='col-sm-12 col-md-12 grid-margin stretch-card'>
                    <div className='card'>
                ...
                {this.state.tutorial === 1 && <div className='col-12 grid-margin stretch-card'>
                    <div className='card'>
                        <div className='mt-4'>
                            <ul className='list-arrow text-'>
                                <li className='lead'> Kiosk Tanimlanamak icin asagida yazan kodu eklemek istediginiz cihazda giriniz</li>
                                <li className='lead'> Kiosk cihazinizda taryinicinizi acin ve bu <span className='text-success'>{`${this.state.hostName}/kiosk/ekle`}</span> linki aciniz </li>
                        ...
                        <div className='card-body'>
                            <div className="form-group">
                                ...
                                <input type="text" className="form-control" value={this.state.kioskName} onChange={(e)=>this.setState({kioskName: e.target.value})} placeholder="Kiosk Name"/>
                            </div>
